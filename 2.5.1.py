...
class SettingsPage(QWidget):
    ...
    def autoufinished(self):
        ...
        if b or c or e or rrr:
            ...
            messageboxall("警告","更新时系统进程环境存在风险\n请注意文件安全性",'w')
...
class UserManager:
    ...
    def __init__(self):
        ...
        self._init_database()
        self.load_session()  # 恢复上次登录会话(使用 QSettings)
    ...
    def add_history(self, image_path: str, text: str, time_taken: str):
        ...
        try:
            cursor.execute('''
                INSERT INTO history (username, timestamp, image_path, text, time_taken)
                VALUES (?, ?, ?, ?, ?)
            ''', (
                self.current_user,
                datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
    ...
    def get_history(self):
        ...
        cursor = conn.cursor()
        cursor.execute('''
            SELECT timestamp, image_path, text, time_taken
            FROM history
            WHERE username = ?
            ORDER BY timestamp DESC
        ''', (self.current_user,))
...
class MainWindow(QMainWindow):
    ...
    def show_login(self):
        ...
        if self.user_manager.current_user:
            ...
            # experimental_captcha = provemankindBW6.MacDifficultCaptchaDialog(self) # 实验性
            exp=QSettings()
